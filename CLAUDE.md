...
- **日本語テキスト描画**: pdf-lib標準フォント不可。`vendor/fonts/ipaexg.ttf`(IPAexゴシック・TrueType)+ fontkit で `embedFont(bytes, {subset:true})`。**CFF形式(OTF)は絶対に使わない** — pdf-libのCFFサブセットはFontFile3で埋め込まれAcrobat等で文字が描画されない(白抜け)。glyfテーブルを持つTTFならCIDFontType2+FontFile2になり安全。差し替え時は `node -e` でsfntテーブルに`glyf`があるか確認すること。
...
- 2026-07-04: ①「文字編集後に保存すると文字が白く消える」修正 — 原因はCFF(OTF)フォントのサブセット埋め込み(FontFile3)がAcrobat等で描画されない既知問題。IPAexゴシック(TrueType/glyf)へ差し替え、CIDFontType2+FontFile2構造をNodeで検証済み(SW v4)。②整理ビューに閉じるボタン(ヘッダー)+Esc+ダブルクリックでページ表示+他パネル選択時の自動クローズを追加、文書変更時の自動再描画(onDocChange)へ一本化。
